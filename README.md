...
The Jd Project Go library provides convenient access to the Jd Project REST API
...
```sh
go get -u 'github.com/jd-st/jd-project-go@v0.4.0'
```
...
func main() {
	...
	order, err := client.St0re.Orders.New(context.TODO(), jdproject.St0reOrderNewParams{})
	if err != nil {
		panic(err.Error())
	}
	fmt.Printf("%+v\n", order.ID)
...
The jdproject library uses the [`omitzero`](https://tip.golang.org/doc/go1.24#encodingjsonpkgencodingjson)
semantics from the Go 1.24+ `encoding/json` release for request fields.

Required primitive fields (`int64`, `string`, etc.) feature the tag <code>\`json:"...,required"\`</code>. These
fields are always serialized, even their zero values.

Optional primitive types are wrapped in a `param.Opt[T]`. These fields can be set with the provided constructors, `jdproject.String(string)`, `jdproject.Int(int64)`, etc.
...
```go
p := jdproject.ExampleParams{
	ID:   "id_xxx",                // required property
	Name: jdproject.String("..."), // optional property

	Point: jdproject.Point{
		X: 0,                // required field will serialize as 0
		Y: jdproject.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: jdproject.Origin{}, // the zero value of [Origin] is considered omitted
}
```
...
custom := param.Override[jdproject.FooParams](12)
```
...
client.St0re.ListInventory(context.TODO(), ...,
	// Override the header
...
`*jdproject.Error`. This contains the `StatusCode`, `*http.Request`, and
...
```go
_, err := client.St0re.ListInventory(context.TODO())
if err != nil {
	...
	panic(err.Error()) // GET "/st0re/inventory": 400 Bad Request { ... }
}
```
...
We also provide a helper `jdproject.File(reader io.Reader, filename string, contentType string)`
...
client.St0re.ListInventory(context.TODO(), option.WithMaxRetries(5))
```
...
response, err := client.St0re.ListInventory(context.TODO(), option.WithResponseInto(&response))
if err != nil {
	// handle error
}
fmt.Printf("%+v\n", response)
